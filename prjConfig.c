...
IMPORT STATUS isrShowInit (void);
...
void usrKernelExtraInit (void)
    ...
    mqPxLibInit (MQ_HASH_SIZE);         /* This is the primary component for support of the message queue POSIX facility for both the kernel and RTPs. POSIX message queues are similar to VxWorks message queues, except that POSIX message queues provide messages with a range of priorities. For more information, see the POSIX facilities section in the VxWorks Programmer's Guide. */
    pthreadLibInit (KERN_PTHREAD_STACK_SIZE_DEFAULT); /* This component includes support for POSIX threads in the kernel. */
    ...
    sigqueueInit (NUM_SIGNAL_QUEUES);   /* This component automatically initializes POSIX queued signals with sigqueueInit(). The sigqueueInit() function allocates buffers for use by sigqueue(), which requires a buffer for each currently queued signal. A call to sigqueue() fails if no buffer is available. */
    ...
    sysPlatformClockInit ();            /* Synchronize VxSim clock with host one */
    }
...
void usrIosCoreInit (void)
    {
    iosPathResStrictInit ();            /* Stricter POSIX pathname handling and support for VRFS-based device tree */
...
void usrShowInit (void)
    ...
    isrShowInit ();                     /* This component provides support for the isrShow() function. It shows information about an ISR object. */
    ...
    mqPxShowInit ();                    /* This component provides the mqPxShow library. This library provides a show routine for displaying information on POSIX message queue objects. The information displayed is for debugging purposes and is intended to be a snapshot of the system at the time the call is made. The mqPxShow() function displays message queue internals. The mqPxShowInit() function initializes the POSIX message queue show facility. */
    ...
    dataCollectorShowInit ();           /* This component provides the dataCollectorShow library which contains routines to display properties of registered dataCollectors. */
    ...
    mmanShowInit();                     /* This component provides show routines for memory mapped objects. */
...
void usrShellCmdInit (void)
    ...
    edrShellCmdInit ();                 /* This component provides shell commands related to error detection and reporting: edr show, edr show fatal, edr show info, edr show interrupt, edr show init, edr show reboot, edr show boot, edr show kernel, edr show user, edr show rtp, edr clear. Aliases: edrs (edr show), edrc (edr clear). */
    ...
    mmanShowShellCmdInit ();            /* This component provides the 'mmap list' shell command which displays information on memory mapped objects. */
    ...
    spyShellCmdInit ();                 /* This component provides shell commands for the spy library: spy, spy stop, spy report, spy clkstart, spy clkstop. Aliases: spy start (spy). */
...
void usrToolsInit (void)
    {
    spyLibInit (SPY_TASKS_MAX);         /* utility to measure per-task CPU utilization. NOTE: the spy need to use aux clock, it may not work properly when workbench profiler is also using aux clock at the same time. */
    ...
    usrSmaInit ();                      /* Support for stop mode debug agent. This agent does not provide a secured network connection; it is advised to not include it in your development system or make sure it is not automatically started at boot time through boot line parameter or dedicated component. */
                                        /* Automatically start the stop mode debug agent at boot. For security reasons, it is advisable for production systems to reconfigure VxWorks with only those components needed for deployed operation. */
    }
...
void usrIosExtraInit (void)
    ...
    aioPxLibInit (MAX_LIO_CALLS);       /* POSIX asynchronous I/O */
    ...
    usrRamDiskInit (RAM_DISK_BLK_SIZE, RAM_DISK_SIZE, RAM_DISK_PART_SUPPORT, RAM_DISK_DEV_NAME); /* This component creates an XBD-based RAM disk. */
    usrBootLineParse ((char *)BOOT_LINE_ADRS); /* This component provides support to parse some boot device configuration information. */
    usrPassFsInit (TRUE);               /* direct access to host filesystem */
...
void usrScInit (void)
    ...
    shlScLibInit ();                    /* system call handlers for shared libraries */
                                        /* library for RTP I/O system limits */
...
void _WRS_NO_STACK_PROTECTOR usrRoot (char *pMemPoolStart, unsigned memPoolSize)
    ...
    usrPmInit ();                       /* This component includes a reboot-safe protected memory region manager. */
    usrEdrInit ();                      /* This component includes a reboot-safe protected error log. */
    ...
    mmanFdLibInit();                    /* This component supports POSIX memory mapped files. */
    usrKernelExtraInit ();              /* Extended kernel facilities */
    usrIosExtraInit ();                 /* extended I/O system */
    usrHostnameSetup (TARGET_HOSTNAME_DEFAULT); /* This component assigns a local hostname to a target. */
...
void _WRS_NO_STACK_PROTECTOR usrInit (int startType)
    ...
    vxFdtShowInit ();                   /* This component provides functions to print out information in the flattened device tree (FDT). */
