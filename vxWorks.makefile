...
target_vxWorks.bin : target_vxWorks
	...
	if [ -n "$(ROMFS_CONTENT)" ]; then \
		...
		$(VXPRJ_BUILDCMD) BUILD_SPEC=$(BUILD_SPEC) SUB_TARGETS="$(SUB_TARGETS)" WS_ROOT_DIR="$(WS_ROOT_DIR)" $(VXWORKS_BIN) ;\
	fi
...
target_vxWorks.hex : target_vxWorks
	...
	if [ -n "$(ROMFS_CONTENT)" ]; then \
		...
		$(VXPRJ_BUILDCMD) BUILD_SPEC=$(BUILD_SPEC) SUB_TARGETS="$(SUB_TARGETS)" WS_ROOT_DIR="$(WS_ROOT_DIR)" $(VXWORKS_HEX) ;\
	fi
...
target_vxWorks% : target_vxWorks
	...
	if [ -n "$(ROMFS_CONTENT)" ]; then \
		...
		$(VXPRJ_BUILDCMD) BUILD_SPEC=$(BUILD_SPEC) SUB_TARGETS="$(SUB_TARGETS)" WS_ROOT_DIR="$(WS_ROOT_DIR)" vxWorks$* ;\
	fi

target_%VxWorks : check_objectdir $(CONFIG_FILE_LIST) subdirs_all
	@echo "building $*VxWorks image of `pwd`";\
	cd $(OBJ_DIR);\
	if [ -n "$(ROMFS_CONTENT)" ]; then \
		$(VXPRJ_BUILDCMD) BUILD_SPEC=$(BUILD_SPEC) ROMFS_CONTENT="$(ROMFS_CONTENT)" SUB_TARGETS="$(SUB_TARGETS)" WS_ROOT_DIR="$(WS_ROOT_DIR)" $*VxWorks ;\
